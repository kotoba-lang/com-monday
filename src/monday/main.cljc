(ns monday.main
  "Kotodama WASM entrypoint for the Monday clean-room actor (L4) — Clojure port.
  ...
  facts (`monday.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "monday")
(def tier "L4")
...
(def entity-specs
  [{:entity "Board"   :plural "boards"   :id-prefix "monday_boa"
    :fields [:name :description :state :boardKind :itemsCount] :required [:name :description]
    :coerce {:itemsCount :int}        :refs {}}
   {:entity "Item"    :plural "items"    :id-prefix "monday_ite"
    :fields [:name :email :creatorId :state]      :required [:name :email]
    :coerce {}                       :refs {}}
   {:entity "Column"  :plural "columns"  :id-prefix "monday_col"
    :fields [:title :description :type :archived :width] :required [:title :description]
    :coerce {:archived :bool :width :int} :refs {}}
   {:entity "Group"   :plural "groups"   :id-prefix "monday_gro"
    :fields [:title :color :position :archived]    :required [:title :color]
    :coerce {:archived :bool}        :refs {}}
   {:entity "User"    :plural "users"    :id-prefix "monday_use"
    :fields [:name :email :enabled :isAdmin :isGuest :countryCode] :required [:name :email]
    :coerce {:enabled :bool :isAdmin :bool :isGuest :bool} :refs {}}
   {:entity "Update"  :plural "updates"  :id-prefix "monday_upd"
    :fields [:body :textBody :creatorId :itemId]   :required [:body :textBody]
    :coerce {}                       :refs {:itemId "Item"}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"monday.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "monday-compat" :tier tier :entities entities} 200])
...
(defn start! [] :monday-compat/ready)
